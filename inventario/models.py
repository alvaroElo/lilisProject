...
from maestros.models import Producto, UnidadMedida, Proveedor
...
class Lote(models.Model):
    ...
    cantidad_reservada = models.DecimalField(max_digits=18, decimal_places=6, default=Decimal('0'))
    bodega = models.ForeignKey(Bodega, on_delete=models.CASCADE)
    proveedor = models.ForeignKey(Proveedor, on_delete=models.PROTECT, null=True, blank=True, 
                                help_text='Proveedor del lote')
...
class MovimientoInventario(models.Model):
    ...
    documento_padre_tipo = models.CharField(max_length=30, choices=DOCUMENTO_PADRE_CHOICES, 
...
class StockActual(models.Model):
    producto = models.ForeignKey(Producto, on_delete=models.CASCADE)
    bodega = models.ForeignKey(Bodega, on_delete=models.CASCADE)
    cantidad_disponible = models.DecimalField(max_digits=18, decimal_places=6, default=Decimal('0'))
    cantidad_reservada = models.DecimalField(max_digits=18, decimal_places=6, default=Decimal('0'))
    cantidad_transito = models.DecimalField(max_digits=18, decimal_places=6, default=Decimal('0'))
    ...
    def __str__(self):
        return f"{self.producto.sku} - {self.bodega.codigo}: {self.cantidad_disponible}"
...
class AlertaStock(models.Model):
    ...
    def __str__(self):
        return f"{self.get_tipo_alerta_display()} - {self.producto.sku}"
